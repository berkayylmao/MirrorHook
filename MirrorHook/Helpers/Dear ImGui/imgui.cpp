...
/*
 ...
 cases you need to pass on the inputs to Dear ImGui.
 - Refer to the FAQ for more information. Amusingly, it is called a FAQ because people frequently
 ...
 radius any more.
 - 2019/12/17 (1.75) - made Columns() limited to 64 columns by asserting above that limit. While the
 ...
 share, remote display) Q: I integrated Dear ImGui in my engine and the text or lines are blurry..
 ...
 (github.com/ocornut/imgui/issues/2847). Visuals are ideal as they inspire other programmers. But
...
static const float WINDOWS_MOUSE_WHEEL_SCROLL_LOCK_TIMER =
    2.00f;  // Lock scrolled window (so it doesn't pick child windows that are scrolling through)
            // for a certaint time, unless mouse moved.
...
void ImGuiIO::AddInputCharacter(unsigned int c) {
  InputQueueCharacters.push_back(
      c > 0 && c <= IM_UNICODE_CODEPOINT_MAX ? (ImWchar)c : IM_UNICODE_CODEPOINT_INVALID);
}
...
bool ImGuiListClipper::Step() {
  ...
  if (StepNo ==
      2)  // Step 2: dummy step only required if an explicit items_height was passed to constructor
...
// behavior will change. Also see imgui_draw.cpp for some more which have been reworked to not rely
// on ImGui:: state.
//-----------------------------------------------------------------------------
...
bool ImGui::BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border,
  ...
  if (g.NavActivateId == id && !(flags & ImGuiWindowFlags_NavFlattened) &&
    ...
    SetActiveID(id + 1, child_window);  // Steal ActiveId with a dummy id so that key-press won't
                                        // activate child item
...
static ImVec2 CalcWindowAutoFitSize(ImGuiWindow* window, const ImVec2& size_contents) {
  ...
  if (window->Flags & ImGuiWindowFlags_Tooltip) {
    ...
    if (is_popup || is_menu)  // Popups and menus bypass style.WindowMinSize by default, but we give
                              // then a non-zero minimum size to facilitate understanding
...
bool ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags) {
  ...
  if (first_begin_of_the_frame) {
    ...
    window->DC.NavFocusScopeIdCurrent =
        parent_window ? parent_window->DC.NavFocusScopeIdCurrent : 0;
...
void ImGui::FocusWindow(ImGuiWindow* window) {
  ...
  ImGuiWindow* focus_front_window =
      window->RootWindow;  // NB: In docking branch this is window->RootWindowDockStop
  ImGuiWindow* display_front_window = window->RootWindow;

  // Steal focus on active widgets
  if (focus_front_window->Flags &
      ImGuiWindowFlags_Popup)  // FIXME: This statement may be unnecessary? Need further testing
                               // before removing it..
...
static ImVec2 CalcNextScrollFromScrollTargetAndClamp(ImGuiWindow* window, bool snap_on_edges) {
  ...
  if (window->ScrollTarget.y < FLT_MAX) {
    ...
    // WindowPadding so that scrolling to make the last item visible scroll far enough to see the
    // padding.
...
bool ImGui::BeginPopupContextWindow(const char* str_id, ImGuiMouseButton mouse_button,
