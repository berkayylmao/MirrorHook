...
// codebase.
// - Call and read ImGui::ShowDemoWindow() in imgui_demo.cpp for demo code. All applications in
...
// - Gallery               https://github.com/ocornut/imgui/issues/2847 (please post your
...
#define IM_UNICODE_CODEPOINT_MAX \
  (sizeof(ImWchar) == 2 ? 0xFFFF : 0x10FFFF)  // Last Unicode code point supported by this build.
...
// the names in the central column below to find the actual flags/enum lists.
#ifndef ImTextureID
...
#endif
typedef unsigned short ImWchar16;  // A single U16 character for keyboard input/display. We encode
                                   // them as multi bytes UTF-8 when used in strings.
typedef unsigned int ImWchar32;    // A single U32 character for keyboard input/display. Define
                                   // ImWchar to ImWchar32 to use it. See imconfig.h .
...
namespace ImGui {
  ...
  // to CreateContext() to share a font atlas between imgui contexts. None of those functions is
  ...
  IMGUI_API void ShowDemoWindow(
      bool* p_open = NULL);  // create Demo window (previously called ShowTestWindow). demonstrate
                             // most ImGui features. call this to learn about the library! try to
  ...
  IMGUI_API void ShowMetricsWindow(
      bool* p_open =
          NULL);  // create Debug/Metrics window. display Dear ImGui internals: draw commands (with
  ...
  IMGUI_API const char* GetVersion();  // get the compiled version string e.g. "1.23" (essentially
                                       // the compiled value for IMGUI_VERSION)
  ...
  //   [as with Begin: this is due to legacy reason and inconsistent with most BeginXXX functions
  //   apart from the regular Begin() which behaves like BeginChild().]
  ...
  IMGUI_API bool IsWindowHovered(
      ImGuiHoveredFlags flags =
          0);  // is current window hovered (and typically: not blocked by a popup/modal)? see flags
               // for options. NB: If you are trying to check whether your mouse should be
  ...
  IMGUI_API void SetNextWindowContentSize(
      const ImVec2& size);  // set next window content size (~ scrollable client area, which enforce
                            // the range of scrollbars). Not including window decorations (title
                            // bar, menu bar, etc.) nor WindowPadding. set an axis to 0.0f to leave
  ...
  GetWindowContentRegionMax();  // content boundaries max (roughly (0,0)+Size-Scroll) where Size can
                                // be override with SetNextWindowContentSize(), in window
                                // coordinates
  ...
  IMGUI_API void PushItemWidth(
      float
          item_width);  // push width of items for common large "item+label" widgets. >0.0f: width
                        // in pixels, <0.0f align xx pixels to the right of window (so -1.0f always
  ...
  IMGUI_API void SetNextItemWidth(
      float item_width);  // set width of the _next_ common large "item+label" widget. >0.0f: width
                          // in pixels, <0.0f align xx pixels to the right of window (so -1.0f
  ...
  // right of the preceeding widget.
  ...
  IMGUI_API bool InvisibleButton(
      const char* str_id,
      const ImVec2&
          size);  // button behavior without the visuals, frequently useful to build custom
  ...
  IMGUI_API void
  Bullet();  // draw a small circle and keep the cursor on the same line. advance cursor x position
  ...
  IMGUI_API bool SliderFloat(
      ...
      float power =
          1.0f);  // adjust format to decorate the value with a prefix or a suffix for in-slider
  ...
  // Widgets: Color Editor/Picker (tip: the ColorEdit* functions have a little colored preview
  // square that can be left-clicked to open a picker, and right-clicked to open an option menu.)
  ...
  IMGUI_API bool ListBoxHeader(
      ...
      const ImVec2& size = ImVec2(0,
                                  0));  // use if you want to reimplement ListBox() will custom data
  ...
  // (*) You can use IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup) to bypass it and
  ...
  IMGUI_API bool BeginPopupContextItem(
      ...
      ImGuiMouseButton mouse_button =
          1);  // helper to open and begin popup when clicked on last item. if you can pass a NULL
               // str_id only if the previous item had an id. If you want to use that on a
               // non-interactive item such as Text() you need to pass in an explicit ID here. read
               // comments in .cpp!
  ...
  IMGUI_API bool BeginPopupContextVoid(
      const char*      str_id       = NULL,
      ImGuiMouseButton mouse_button = 1);  // helper to open and begin popup when clicked in void
  ...
  IMGUI_API bool OpenPopupOnItemClick(
      const char*      str_id = NULL,
      ImGuiMouseButton mouse_button =
          1);  // helper to open popup when clicked on last item (note: actually triggers on the
               // mouse _released_ event to be consistent with popup behaviors). return true when
               // just opened.
  IMGUI_API bool IsPopupOpen(const char* str_id);  // return true if the popup is open at the
                                                   // current begin-ed level of the popup stack.
  IMGUI_API void
  ...
  // - The columns API is work-in-progress and rather lacking (columns are arguably the worst part
  // of dear imgui at the moment!)
  // - There is a maximum of 64 columns.
  // - Currently working on new 'Tables' api which will replace columns around Q2 2020 (see GitHub
  // #2957).
  ...
  IMGUI_API bool IsItemClicked(ImGuiMouseButton mouse_button =
                                   0);  // is the last item clicked? (e.g. button/node just clicked
  ...
  IMGUI_API        ImDrawList*
                   GetBackgroundDrawList();  // this draw list will be the first rendering one. Useful to quickly
  ...
  IMGUI_API bool IsMouseDoubleClicked(
      ImGuiMouseButton button);  // did mouse button double-clicked? a double-click returns false in
  ...
  // Clipboard Utilities (also see the LogToClipboard() function to capture or output text data to
  // the clipboard)
...
enum ImGuiWindowFlags_ {
  ...
  ImGuiWindowFlags_UnsavedDocument =
      1 << 20,  // Append '*' to title without affecting the ID, as a convenience to avoid using the
                ...
                // closure is deferred by one frame to allow code to cancel the closure (with a
  ...
  // style.WindowBorderSize=1.0f to enable borders around items or windows.
  // ImGuiWindowFlags_ResizeFromAnySide    = 1 << 17,  // --> Set
  // io.ConfigWindowsResizeFromEdges=true and make sure mouse cursors are supported by back-end
...
enum ImGuiHoveredFlags_ {
  ...
  // ImGuiHoveredFlags_AllowWhenBlockedByModal     = 1 << 4,   // Return true even if a modal popup
...
enum ImGuiBackendFlags_ {
  ...
  ImGuiBackendFlags_HasMouseCursors = 1
                                      << 1,  // Back-end Platform supports honoring GetMouseCursor()
                                             // value to change the OS cursor shape.
  ImGuiBackendFlags_HasSetMousePos =
      1 << 2,  // Back-end Platform supports io.WantSetMousePos requests to reposition the OS mouse
...
//since 1.60+] the close button now uses regular button colors.
#endif
};
...
struct ImGuiStyle {
  ...
  ImVec2 WindowMinSize;  // Minimum window size. This is a global setting. If you want to constraint
  ...
  ImVec2 ButtonTextAlign;  // Alignment of button text when button is larger than text. Defaults to
                           // (0.5f, 0.5f) (centered).
  ImVec2 SelectableTextAlign;   // Alignment of selectable text when selectable is larger than text.
                                // Defaults to (0.0f, 0.0f) (top-left aligned).
  ImVec2 DisplayWindowPadding;  // Window position are clamped to be visible within the display area
  ...
  float CircleSegmentMaxError;  // Maximum error (in pixels) allowed when using
                                ...
                                // rectangles with no explicit segment count specified. Decrease for
...
struct ImGuiIO {
  ...
  bool ConfigMacOSXBehaviors;  // = defined(__APPLE__) // OS X style: Text editing cursor movement
                               ...
                               // text, Multi-selection in lists uses Cmd/Super instead of Ctrl (was
  ...
  float ConfigWindowsMemoryCompactTimer;   // = 60.0f          // [BETA] Compact window memory usage
  ...
  bool MouseDown[5];  // Mouse buttons: 0=left, 1=right, 2=middle + extras. ImGui itself mostly only
  ...
  float NavInputs[ImGuiNavInput_COUNT];  // Gamepad inputs. Cleared back to zero by EndFrame().
                                         // Keyboard keys will be auto-mapped and be written here by
                                         // NewFrame().
  ...
  bool WantCaptureMouse;  // When io.WantCaptureMouse is true, imgui will use the mouse inputs, do
                          // not dispatch them to your main game/application (in both cases, always
                          // pass on mouse inputs to imgui). (e.g. unclicked mouse is hovering over
                          // an imgui window, widget is active, mouse was clicked over an imgui
                          // window, etc.).
  bool
      WantCaptureKeyboard;  // When io.WantCaptureKeyboard is true, imgui will use the keyboard
                            // inputs, do not dispatch them to your main game/application (in both
                            // cases, always pass keyboard inputs to imgui). (e.g. InputText active,
  ...
  bool WantSaveIniSettings;  // When manual .ini load/save is active (io.IniFilename == NULL), this
                             ...
                             // SaveIniSettingsToMemory() and save yourself. IMPORTANT: You need to
                             // clear io.WantSaveIniSettings yourself.
  ...
  ImVec2 MouseDragMaxDistanceAbs[5];  // Maximum distance, absolute, on each axis, of how much mouse
...
struct ImGuiListClipper {
  ...
  // GetFrameHeightWithSpacing(). If you don't specify an items_height, you NEED to call Step(). If
  // you specify items_height you may call the old Begin()/End() api directly, but prefer calling
  // Step().
  ImGuiListClipper(int items_count = -1, float items_height = -1.0f) {
    Begin(items_count, items_height);
  }  // NB: Begin() initialize every fields (as we allow user to call Begin/End multiple times on a
     // same instance if they want).
  ~ImGuiListClipper() {
...
// than 64K vertices while keeping 16-bit indices.
struct ImDrawCmd {
  unsigned int
...
enum ImDrawListFlags_ {
  ...
  ImDrawListFlags_AllowVtxOffset = 1
                                   << 2  // Can emit 'VtxOffset > 0' to allow large meshes. Set when
...
struct ImDrawList {
  ...
  //   Use AddNgon() and AddNgonFilled() functions if you need to guaranteed a specific number of
  //   sides.
  ...
  IMGUI_API void AddPolyline(const ImVec2* points, int num_points, ImU32 col, bool closed,
                             float thickness);
  ...
  IMGUI_API void AddBezierCurve(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
  ...
  IMGUI_API void PathBezierCurveTo(const ImVec2& p2, const ImVec2& p3, const ImVec2& p4,
                                   int num_segments = 0);
...
struct ImFontAtlasCustomRect {
  unsigned int ID;  // Input    // User ID. Use < 0x110000 to map into a font glyph, >= 0x110000 for
  ...
  ImVec2
          GlyphOffset;  // Input    // For custom font glyphs only (ID < 0x110000): glyph display offset
...
struct ImFontAtlas {
  ...
  // In C++11, you can create UTF-8 string literal using the u8"Hello world" syntax. See FAQ for
  ...
  IMGUI_API int AddCustomRectRegular(unsigned int id, int width,
                                     int height);  // Id needs to be >= 0x110000. Id >= 0x80000000
  ...
  IMGUI_API int AddCustomRectFontGlyph(
      ...
      const ImVec2& offset = ImVec2(
          0, 0));  // Id needs to be < 0x110000 to register a rectangle to map into a specific font.
...
struct ImFont {
  ...
  ImU8 Used4kPagesMap[(IM_UNICODE_CODEPOINT_MAX + 1) / 4096 /
                      8];  // 2 bytes if ImWchar=ImWchar16, 34 bytes if ImWchar==ImWchar32. Store
                           ...
                           // is mainly used to facilitate iterations accross all used codepoints.
