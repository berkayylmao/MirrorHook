...
bool ImGui::CloseButton(ImGuiID id, const ImVec2& pos)  //, float size)
{
  ...
  // sequence close a window. (this isn't the regular behavior of buttons, but it doesn't affect the
  // user much because navigation tends to keep items visible).
...
bool ImGui::CollapseButton(ImGuiID id, const ImVec2& pos) {
  ...
  ImVec2 center   = bb.GetCenter();
  if (hovered || held)
    window->DrawList->AddCircleFilled(center /*+ ImVec2(0.0f, -0.5f)*/, g.FontSize * 0.5f + 1.0f,
                                      bg_col, 12);
...
bool ImGui::ScrollbarEx(const ImRect& bb_frame, ImGuiID id, ImGuiAxis axis, float* p_scroll_v,
                        float size_avail_v, float size_contents_v,
...
bool ImGui::ImageButton(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0,
  ...
  // We could hash the size/uv to create a unique ID but that would prevent the user from animating
  // UV.
...
void ImGui::DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg1,
                            const void* arg2) {
  IM_ASSERT(op == '+' || op == '-');
  switch (data_type) {
...
bool ImGui::DragBehaviorT(ImGuiDataType data_type, TYPE* v, float v_speed, const TYPE v_min,
  ...
  if (is_power) {
    // Offset + round to user desired precision, with a curve on the v_min..v_max range to get more
    ...
    v_cur =
        v_min + (SIGNEDTYPE)ImPow(ImSaturate((float)v_new_norm_curved), power) * (v_max - v_min);
...
// widget, p_min and p_max are optional. Read code of e.g. SliderFloat(), SliderInt() etc. or
// examples in 'Demo->Widgets->Data Types' to understand how to use this function directly.
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed,
  ...
  if (format == NULL)
    format = DataTypeGetInfo(data_type)->PrintFmt;
           ...
           strcmp(format, "%d") != 0)  // (FIXME-LEGACY: Patch old "%.0f" format string to use "%d",
...
bool ImGui::SliderBehaviorT(const ImRect& bb, ImGuiID id, ImGuiDataType data_type, TYPE* v,
  ...
  // For power curve sliders that cross over sign boundary we want the curve to be symmetric around
  // 0.0f
    ...
    if (set_new_value) {
      ...
      if (is_power) {
        ...
        if (is_decimal) {
          ...
          // above This code is carefully tuned to work with large values (e.g. high ranges of U64)
...
bool ImGui::TempInputScalar(const ImRect& bb, ImGuiID id, const char* label,
  ...
  if (value_changed) {
    value_changed = DataTypeApplyOpFromText(data_buf, g.InputTextState.InitialTextA.Data, data_type,
                                            p_data, NULL);
    if (value_changed) MarkItemEdited(id);
  }
  return value_changed;
...
bool ImGui::InputScalar(const char* label, ImGuiDataType data_type, void* p_data,
  ...
  flags |= ImGuiInputTextFlags_NoMarkEdited;  // We call MarkItemEdited() ourselve by comparing the
  ...
  if (p_step != NULL) {
    ...
    if (InputText("", buf, IM_ARRAYSIZE(buf),
      ...
      value_changed = DataTypeApplyOpFromText(buf, g.InputTextState.InitialTextA.Data, data_type,
                                              p_data, format);
...
void ImGuiInputTextCallbackData::InsertChars(int pos, const char* new_text,
  ...
  if (new_text_len + BufTextLen >= BufSize) {
    ...
    // Contrary to STB_TEXTEDIT_INSERTCHARS() this is working in the UTF8 buffer, hence the midly
    // similar code (until we remove the U16 buffer alltogether!)
    ImGuiContext&        g          = *GImGui;
...
bool ImGui::InputTextEx(const char* label, const char* hint, char* buf, int buf_size,
  ...
  if (init_state && g.ActiveId != id) {
    ...
    // FIXME: For non-readonly widgets we might be able to require that TextAIsValid && TextA == buf
    // ? (untested) and discard undo stack if user buffer has changed.
  ...
  if (g.ActiveId != id && init_make_active) {
    ...
    if (is_multiline)
      g.ActiveIdUsingKeyInputMask |=
          ...
          ((ImU64)1 << ImGuiKey_PageDown);  // FIXME-NAV: Page up/down actually not supported yet by
                                            // widget, but claim them ahead.
...
bool ImGui::ColorEdit4(const char* label, float col[4], ImGuiColorEditFlags flags) {
  ...
  flags |=
      (g.ColorEditOptions & ~(ImGuiColorEditFlags__DisplayMask | ImGuiColorEditFlags__DataTypeMask |
  ...
  if ((flags & (ImGuiColorEditFlags_DisplayRGB | ImGuiColorEditFlags_DisplayHSV)) != 0 &&
    ...
    const float w_item_one = ImMax(
        1.0f,
        IM_FLOOR((w_inputs - (style.ItemInnerSpacing.x) * (components - 1)) / (float)components));
  ...
  if ((flags & ImGuiColorEditFlags_NoInputs) == 0) {
    ...
    if (flags & ImGuiColorEditFlags_DisplayRGB || (flags & ImGuiColorEditFlags__DisplayMask) == 0)
      if (ColorEdit4("##rgb", col, sub_flags | ImGuiColorEditFlags_DisplayRGB)) {
        // FIXME: Hackily differenciating using the DragInt (ActiveId != 0 && !ActiveIdAllowOverlap)
...
bool ImGui::ColorButton(const char* desc_id, const ImVec4& col, ImGuiColorEditFlags flags,
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col_rgb.w < 1.0f) {
    ...
    if (col_source.w < 1.0f)
      RenderColorRectWithAlphaCheckerboard(bb_inner.Min, bb_inner.Max, GetColorU32(col_source),
...
void ImGui::SetColorEditOptions(ImGuiColorEditFlags flags) {
  ...
  IM_ASSERT(
      ImIsPowerOfTwo(flags & ImGuiColorEditFlags__DisplayMask));  // Check only 1 option is selected
...
bool ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label,
  ...
  const float arrow_hit_x2 =
      (text_pos.x - text_offset_x) + (g.FontSize + padding.x * 2.0f) + hit_padding_x;
  ...
  if (!is_leaf) {
    if (pressed) {
      ...
      if ((flags & ImGuiTreeNodeFlags_OpenOnDoubleClick) && g.IO.MouseDoubleClicked[0])
        toggled = true;
      if (g.DragDropActive && is_open)  // When using Drag and Drop "hold to open" we keep the node
  ...
  if (display_frame) {
    ...
    if (g.LogEnabled) {
      // NB: '##' is normally used to hide text (as a library-wide feature), so we need to specify
      // the text range to make sure the ## aren't stripped out here.
...
bool ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags,
  ...
  ImVec2 size_draw(
      (size_arg.x != 0 && !(flags & ImGuiSelectableFlags_DrawFillAvailWidth)) ? size_arg.x : w_draw,
  ...
  RenderTextClipped(bb_inner.Min, bb_inner.Max, label, NULL, &label_size, style.SelectableTextAlign,
                    &bb);
...
bool ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items) {
  ...
  // widget resize occasionally when it crosses that size. I am expecting that someone will come and
...
void ImGui::EndMenuBar() {
  ...
  window->DC.MenuBarOffset.x =
      ...
      window->MenuBarRect().Min.x;  // Save horizontal position so next append can reuse it. This is
...
bool ImGui::BeginMenu(const char* label, bool enabled) {
  ...
  // this is ever becoming a problem we can switch to use e.g. a ImGuiStorager mapping key to last
  // frame used.
  ...
  // for the child menu, However the final position is going to be different! It is choosen by
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    float w   = window->DC.MenuColumns.DeclColumns(
                                 ...
                                 // (ImGuiWindowFlags_Popup|ImGuiWindowFlags_ChildMenu))
  {
    ...
    if (g.HoveredWindow == window && child_menu_window != NULL &&
      ...
      ta.x +=
          (window->Pos.x < child_menu_window->Pos.x) ? -0.5f : +0.5f;  // to avoid numerical issues
...
bool ImGui::MenuItem(const char* label, const char* shortcut, bool selected, bool enabled) {
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    // a little misleading but may be useful Note that in this situation we render neither the
    // shortcut neither the selected tick mark
    float w = label_size.x;
    window->DC.CursorPos.x += IM_FLOOR(style.ItemSpacing.x * 0.5f);
    PushStyleVar(ImGuiStyleVar_ItemSpacing,
                 ImVec2(style.ItemSpacing.x * 2.0f, style.ItemSpacing.y));
    ...
    float  w             = window->DC.MenuColumns.DeclColumns(
        label_size.x, shortcut_size.x, IM_FLOOR(g.FontSize * 1.20f));  // Feedback for next frame
    float extra_w = ImMax(0.0f, GetContentRegionAvail().x - w);
    pressed =
        Selectable(label, false, flags | ImGuiSelectableFlags_DrawFillAvailWidth, ImVec2(w, 0.0f));
    ...
    if (selected)
      RenderCheckMark(pos + ImVec2(window->DC.MenuColumns.Pos[2] + extra_w + g.FontSize * 0.40f,
...
bool ImGui::BeginTabBarEx(ImGuiTabBar* tab_bar, const ImRect& tab_bar_bb, ImGuiTabBarFlags flags) {
  ...
  // visible order. Otherwise, the most recently inserted tabs would move at the end of visible list
  // which can be a little too confusing or magic for the user.
...
bool ImGui::BeginTabItem(const char* label, bool* p_open, ImGuiTabItemFlags flags) {
  ...
  if (tab_bar == NULL) {
    IM_ASSERT_USER_ERROR(
        tab_bar, "BeginTabItem() Needs to be called between BeginTabBar() and EndTabBar()!");
    return false;
  }
...
bool ImGui::TabItemEx(ImGuiTabBar* tab_bar, const char* label, bool* p_open,
  ...
  // call to ItemAdd() so that attempts to use a contextual popup menu with an implicit ID won't use
  // an older ID.
  if (p_open && !*p_open) {
  ...
  bool just_closed = TabItemLabelAndCloseButton(display_draw_list, bb, flags, tab_bar->FramePadding,
                                                label, id, close_button_id);
  ...
  // HoveredIdTimer) We test IsItemHovered() to discard e.g. when another item is active or drag and
...
void ImGui::SetTabItemClosed(const char* label) {
  ...
  if (is_within_manual_tab_bar) {
    ImGuiTabBar* tab_bar = g.CurrentTabBar;
    IM_ASSERT(tab_bar->WantLayout);  // Needs to be called AFTER BeginTabBar() and BEFORE the first
...
bool ImGui::TabItemLabelAndCloseButton(ImDrawList* draw_list, const ImRect& bb,
  ...
  if (flags & ImGuiTabItemFlags_UnsavedDocument) {
    ...
    RenderTextClippedEx(draw_list, unsaved_marker_pos, bb.Max - frame_padding, TAB_UNSAVED_MARKER,
                        NULL, NULL);
  }
