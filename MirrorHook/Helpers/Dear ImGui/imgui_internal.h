...
IMGUI_API ImVec2 ImBezierCalc(const ImVec2& p1, const ImVec2& p2, const ImVec2& p3,
                              const ImVec2& p4, float t);  // Cubic Bezier
...
enum ImGuiButtonFlags_ {
  ...
  ImGuiButtonFlags_PressedOnClickRelease =
      1 << 2,  // [Default] return true on click + release on same item <-- this is what the
               // majority of Button are using
  ImGuiButtonFlags_PressedOnClickReleaseAnywhere =
      1 << 3,  // return true on click + release even if the release event is not done while
               // hovering the item
  ...
  ImGuiButtonFlags_PressedOnDragDropHold =
      1 << 6,  // return true when held into while we are drag and dropping another item (used by
               // e.g. tree nodes, collapsing headers)
  ImGuiButtonFlags_FlattenChildren =
      1 << 7,  // allow interactions even if a child window is overlapping
  ImGuiButtonFlags_AllowItemOverlap =
      1 << 8,  // require previous frame HoveredId to either match id or be null before being
               // usable, use along with SetItemAllowOverlap()
  ImGuiButtonFlags_DontClosePopups =
      1 << 9,  // disable automatically closing parent popup on press // [UNUSED]
  ImGuiButtonFlags_Disabled = 1 << 10,  // disable interactions
  ImGuiButtonFlags_AlignTextBaseLine =
      1 << 11,  // vertically align button to match text baseline - ButtonEx() only // FIXME: Should
...
enum ImGuiSelectableFlagsPrivate_ {
  ...
  ImGuiSelectableFlags_DrawHoveredWhenHeld =
      1 << 24,  // Always show active when held, even is not hovered. This concept could probably be
                // renamed/formalized somehow.
...
struct ImGuiContext {
  ...
  ImU32 ActiveIdUsingNavDirMask;    // Active widget will want to read those directional navigation
                                    // requests (e.g. can activate a button and move away from it)
  ImU32 ActiveIdUsingNavInputMask;  // Active widget will want to read those nav inputs.
  ImU64 ActiveIdUsingKeyInputMask;  // Active widget will want to read those key inputs. When we
                                    // grow the ImGuiKey enum we'll need to either to order the enum
                                    // to make useful keys come first, either redesign this into
                                    // e.g. a small array.
  ...
  ImGuiWindow*
      NavWindowingTarget;  // When selecting a window (holding Menu+FocusPrev/Next, or equivalent of
  ...
  ImGuiWindow* NavWindowingTargetAnim;  // Record of last valid NavWindowingTarget until DimBgRatio
                                        // and NavWindowingHighlightAlpha becomes 0.0f
  ...
  int FocusRequestCurrCounterRegular;  // Any item being requested for focus, stored as an index (we
  ...
  int   WantCaptureMouseNextFrame;  // Explicit capture via
                                  // CaptureKeyboardFromApp()/CaptureMouseFromApp() sets those flags
...
struct IMGUI_API ImGuiWindowTempData {
  ...
  float ItemWidth;  // == ItemWidthStack.back(). 0.0: default, >0.0: width in pixels, <0.0: align xx
...
struct IMGUI_API ImGuiWindow {
  ...
  ImRect WorkRect;  // Cover the whole scrolling region, shrunk by WindowPadding*1.0f on each side.
                    // This is meant to replace ContentRegionRect over time (from 1.71+ onward).
...
namespace ImGui {
  ...
  IMGUI_API void PushOverrideID(ImGuiID id);     // Push given value at the top of the ID stack
  ...
  IMGUI_API bool IsPopupOpen(
      ImGuiID id);  // Test for id within current popup stack level (currently begin-ed into); this
  ...
  // Focus scope (WIP)
  IMGUI_API void PushFocusScope(ImGuiID id);  // Note: this is storing in same stack as IDStack, so
                                              // Push/Pop mismatch will be reported there.
  ...
  IMGUI_API bool    ScrollbarEx(const ImRect& bb, ImGuiID id, ImGuiAxis axis, float* p_scroll_v,
  ...
  IMGUI_API void DataTypeApplyOp(ImGuiDataType data_type, int op, void* output, void* arg_1,
                                 const void* arg_2);
  ...
  inline void DebugDrawItemRect(ImU32 col = IM_COL32(255, 0, 0, 255)) {
    ...
    GetForegroundDrawList(window)->AddRect(window->DC.LastItemRect.Min, window->DC.LastItemRect.Max,
                                           col);
  }
